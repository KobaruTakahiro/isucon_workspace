...
var chache = make(map[string]Chache)
...
func makePosts(results []Post, CSRFToken string, allComments bool) ([]Post, error) {
	...
	for _, p := range results {
		...
		go func() {
			...
			db.Get(&p.CommentCount, "SELECT COUNT(*) AS `count` FROM `comments` WHERE `post_id` = ?", p.ID)
			/*
			if err != nil {
				return nil, err
			}
			...
			*/
			...
			for i := 0; i < len(comments); i++ {
				...
				db.Get(&comments[i].User, "SELECT * FROM `users` WHERE `id` = ?", comments[i].UserID)
				/*
				if uerr != nil {
					return nil, uerr
				}
			...
			db.Get(&p.User, "SELECT * FROM `users` WHERE `id` = ?", p.UserID)
			/*
			if perr != nil {
				return nil, perr
			}
...
func getImage(c web.C, w http.ResponseWriter, r *http.Request) {
	...
	m2 = strconv.Itoa(pid) + ext

	if val, ok := chache[m2]; ok {
		w.Header().Set("Content-Type", val.Mime)
...
func postAdminBanned(w http.ResponseWriter, r *http.Request) {
	...
	idStirng = strings.Join(ids[:], ",")	
	db.Exec(query, 1, idStirng)
